...
public class AjoutAboCtrl {
	...
	public void creerAbo() {
		...
		if(dateDeb == null) {
			...
			id_lb_custom.setTextFill(Color.RED);
			id_lb_custom.setText("La date de fin est sup�rieure � la date de d�but");
		}else {
	...
	public void affiModifAbo() {
		...
		if(selection.size() == 0) {
			id_error_label.setTextFill(Color.RED);
			id_error_label.setText("Aucun abonnement s�lectionn�");
		}else if(selection.size() > 1) {
			id_error_label.setTextFill(Color.RED);
			id_error_label.setText("Plusieurs abonnements s�lectionn�s");
		}else {
			...
			label_abonnement.setText("Modifier l'abonnement du client" + id_client + " � la revue " + id_revue);
